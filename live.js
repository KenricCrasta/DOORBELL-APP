...
export default function Live() {
 ...
 set(liveRef, { live: 1 })
  ...
  const fetchStreamUrl = async () => {
      try {
        // Fetch the stream URL (replace 'YOUR_STREAM_URL' with the actual URL)
        const streamUrl = 'http://192.168.0.106:8000/video_feed';
    ...
    const func = async () => {                                                              //FIREBASE
      try{
        
      const store=getStorage();
      const referen = ref(store,'/LIVE.jpg');
    ...
    return (
      <View style={styles.container}>
        <View style={styles.topbar}>
            <Text style={styles.topbartext}>LIVE</Text>
        </View>
        <View style={styles.image}>
        { isLoading ?(<Image source={{uri: url} } style={styles.img} />):(
